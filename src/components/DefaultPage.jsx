...
export const DefaultPage = ({pageId, personComponent}) => {
    ...
    const scrollToAdvices = () => {
        if (!advicesRef?.current) return;

        advicesRef.current.scrollIntoView({behavior: 'smooth'});
        setIsMenuOpen(false);
    }
    ...
    return (
        <Wrapper>
            <AboutBlock ref={aboutRef}>
                ...
                <AboutVacancies moveToAdvices={scrollToAdvices} vacanciesDescr={vacanciesDescr}/>
            </AboutBlock>
            <AnimatePresence>
                {
            ...
            <SpacingContent>
                ...
                <TestBlock moveToAdvices={scrollToAdvices} testBlockMargin={testBlockMargin} testFullName={testFullName} scrollToVacancy={scrollToVacancy} person={testPerson} testName={testName} questions={testQuestions}/>
                <Addictions ref={advicesRef} testAdditionMargin={testAdditionMargin} isDefault pageId={pageId} onClick={(data) => setModalState(prev =>( {...prev, ...data}))}/>
                <UpButton onClick={() => aboutRef?.current?.scrollIntoView({top: 0, behavior: "smooth"})}>
                    ...
                    <rect x="1" y="41" width="40" height="40" rx="20" transform="rotate(-90 1 41)" stroke="#FF7F00" strokeWidth="2"/>
                    </svg>
                </UpButton>
                <Footer>
                    <FooterText $color={'var(--color-gray)'}>
                        © 2005-2026 FutureToday.{'\n'}Все права защищены.
                    </FooterText>
                    <BotFooterText>
                        <SmallText $color={'var(--color-gray)'}>группа вк</SmallText>
                        <svg width="27" height="15" viewBox="0 0 27 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M1 6.36395C0.447715 6.36395 9.65645e-08 6.81167 0 7.36395C-9.65645e-08 7.91624 0.447715 8.36395 1 8.36395L1 7.36395L1 6.36395ZM26.7071 8.07106C27.0976 7.68054 27.0976 7.04737 26.7071 6.65685L20.3431 0.292888C19.9526 -0.0976362 19.3195 -0.0976364 18.9289 0.292888C18.5384 0.683412 18.5384 1.31658 18.9289 1.7071L24.5858 7.36396L18.9289 13.0208C18.5384 13.4113 18.5384 14.0445 18.9289 14.435C19.3195 14.8255 19.9526 14.8255 20.3431 14.435L26.7071 8.07106ZM1 7.36395L1 8.36395L26 8.36396L26 7.36396L26 6.36396L1 6.36395L1 7.36395Z" fill={'var(--color-gray)'}/>
                        </svg>
            ...
            <AnimatePresence>
                {modalState.shown && (
                    <JobModal onopenVk={handleopenVk} picture={[opportunitiesPerson, opportunitiesPersonDesk]}  opportunities={opportunities} id={modalState.id} onClose={handleCloseModal} />
                )}
            </AnimatePresence>
             <AnimatePresence>
                {isMenuOpen && (
                    <MenuStyled initial={{scale: 0, opacity: 0}} animate={{scale: 1, opacity: 1}} exit={{scale: 0, opacity: 0}} transition={{transformOrigin: '100% 0'}}>
                        <SecondButtonStyled onClick={scrollToOpps}>
                            кем работать
                        </SecondButtonStyled>
                        <SecondButtonStyled onClick={scrollToVacancy}>
                            вакансии
                        </SecondButtonStyled>
                        <SecondButtonStyled onClick={scrollToAdvices}>
                            советы про работу
                        </SecondButtonStyled>
                    </MenuStyled>
                )}
            </AnimatePresence>
            <MaterialModal isOpen={modalState.isSmallModal} onClose={() => setModalState({isSmallModal: false, shown: false})}>
